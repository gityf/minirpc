...
**Description: source file of Closure ut.
*/
...
TEST(ClosureTest, FuncWithoutPrebindTest)
{
    Closure<int, char>* c = NewClosure(foo);
    EXPECT_EQ(c->Run('a'), 5);
}

TEST(ClosureTest, FuncWithPrebindTest)
{
    Closure<int>* c = NewClosure(foo, 'a');
    EXPECT_EQ(c->Run(), 5);
}

TEST(ClosureTest, PermanentFuncWithPrebindTest)
{
    Closure<int>* c = NewPermanentClosure(foo, 'a');
    EXPECT_EQ(c->Run(), 5);
    EXPECT_EQ(c->Run(), 5);
    delete c;
}

TEST(ClosureTest, PermanentFuncWithoutPrebindTest)
{
    Closure<int, char>* c = NewPermanentClosure(foo);
    EXPECT_EQ(c->Run('b'), 5);
    EXPECT_EQ(c->Run('b'), 5);
    delete c;
}

TEST(ClosureTest, MethodWithoutPrebindTest)
{
    SS ss;
    Closure<int, int, const std::string*, double>* c
        = NewClosure(&ss, &SS::bar);
    EXPECT_EQ(c->Run(0, NULL, 1.0), -2);
}

TEST(ClosureTest, MethodWithPrebindTest)
{
    SS ss;
    Closure<int, double>* c
        = NewClosure(&ss, &SS::bar, 1, (const std::string*)NULL);
    EXPECT_EQ(c->Run(1.0), -2);
}

TEST(ClosureTest, PermanentMethodWithPrebindTest)
{
    SS ss;
    Closure<int, double>* c
        = NewPermanentClosure(&ss, &SS::bar, 1, (const std::string*)NULL);
    EXPECT_EQ(c->Run(1.0), -2);
    EXPECT_EQ(c->Run(1.0), -2);
    delete c;
}

TEST(ClosureTest, PermanentMethodWithoutPrebindTest)
{
    SS ss;
    Closure<int, int, const std::string*, double>* c
        = NewPermanentClosure(&ss, &SS::bar);
    EXPECT_EQ(c->Run(0, NULL, 1.0), -2);
    EXPECT_EQ(c->Run(0, NULL, 1.0), -2);
    delete c;
}
