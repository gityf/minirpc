...
{
    return (MD5_CTX*)malloc(3 *sizeof(MD5_CTX));
}
...
void hmacmd5_key(void *handle, void *keyv, int len)
{
    MD5_CTX *keys = (MD5_CTX *)handle;
    unsigned char foo[64];
    unsigned char *key = (unsigned char *)keyv;
    int i;

    MD5_memset(foo, 0x36, 64);
    ...
    MD5_memset(foo, 0, 64);                       /* burn the evidence */
}
...
static void hmacmd5_bytes(void *handle, unsigned char *blk, int len)
{
    MD5_CTX *keys = (MD5_CTX *)handle;
...
static int hmacmd5_verresult(void *handle, unsigned char *hmac)
{
...
void hmacmd5_do_hmac(void *handle, unsigned char *blk, int len,
                     unsigned char *hmac)
{
    hmacmd5_do_hmac_internal(handle, blk, len, 0, 0, hmac);
}

static void hmacmd5_do_hmac_ssh(void *handle, unsigned char *blk, int len,
