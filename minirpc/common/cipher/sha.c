...
static void *sha1_init(void)
{
    ...
    s = (SHA_State*)malloc(sizeof(SHA_State));
    SHA_Init(s);
    return s;
}
...
{
    return (SHA_State *)malloc(3 * sizeof(SHA_State));
}
...
static void hmacsha1_bytes(void *handle, unsigned char *blk, int len)
{
    SHA_State *keys = (SHA_State *)handle;
...
static int hmacsha1_verresult(void *handle, unsigned char *hmac)
{
...
static int hmacsha1_96_verresult(void *handle, unsigned char *hmac)
{
    unsigned char correct[20];
    hmacsha1_genresult(handle, correct);
